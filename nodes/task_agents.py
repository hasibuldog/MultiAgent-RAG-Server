...
def flashcard_agent(state: AgentState) -> AgentState:
    ...
    flashcard_prompt = ChatPromptTemplate.from_messages([
        ("system", "You are a flashcard creation agent. Generate flashcards based on the retrieved documents provided."),
        ("human", "{input}"),
        ("user", "{retrieved_docs}"),
...
def summarizer_agent(state: AgentState) -> AgentState:
    summarization_prompt = ChatPromptTemplate.from_messages([
        ("system", "You are a summarization agent. Summarize the content of the retrieved documents."),
        ('human', "{input}"),
        ("user", "{retrieved_docs}"),
        MessagesPlaceholder(variable_name="agent_scratchpad")
...
def studyplan_agent(state: AgentState) -> AgentState:
    studyplan_prompt = ChatPromptTemplate.from_messages([
        ("system", "You are a study plan generator. Create a study plan based on the retrieved documents."),
        ("human", "{input}"),
        ("user", "{retrieved_docs}"),
        MessagesPlaceholder(variable_name="agent_scratchpad")
    ])
    ...
    studyplan_agent_fn = create_openai_functions_agent(
        llm=model,
        prompt=studyplan_prompt,
        tools=[tool]
    )

    study_plan_executor = AgentExecutor(
        agent=studyplan_agent_fn,
        memory=memory,
        verbose=True,
        tools=[tool]
    )
...
def quiz_agent(state: AgentState) -> AgentState:
    ...
    quiz_prompt = ChatPromptTemplate.from_messages([
        ("system", "You are a quiz question generation agent. Create quiz questions based on the retrieved documents."),
        ("human", "{human_query}"),
        ("user", "{retrieved_docs}"),
        MessagesPlaceholder(variable_name="agent_scratchpad")
    ])
